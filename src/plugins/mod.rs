...
impl PluginManager {
    ...
    fn endpoint_to_dict<'py>(&self, py: Python<'py>, endpoint: &Endpoint) -> Bound<'py, PyDict> {
        ...
        let _ = dict.set_item("url", &endpoint.url);
        let _ = dict.set_item("method", &endpoint.method);
        let _ = dict.set_item(
            "endpoint_type",
            format!("{:?}", endpoint.endpoint_type).to_lowercase(),
        );
        let _ = dict.set_item("source", &endpoint.source);
        let _ = dict.set_item("line", endpoint.line);
        let _ = dict.set_item("params", &endpoint.params);
        dict
    }
    ...
    fn dict_to_endpoint(&self, dict: &Bound<'_, PyDict>) -> Result<Endpoint> {
        let url: String = dict
            .get_item("url")
            .and_then(|item| item.map(|i| i.extract::<String>().ok()).flatten())
